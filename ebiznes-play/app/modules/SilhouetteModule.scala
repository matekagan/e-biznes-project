...
import auth.repository.{OAuth2InfoDaoImpl, PasswordInfoDaoImpl, UserRepository, UserRepositoryImpl}
import auth.services.{UserService, UserServiceImpl}
import auth.{CustomSecuredErrorHandler, CustomUnsecuredErrorHandler}
...
class SilhouetteModule extends AbstractModule with ScalaModule {
  ...
  override def configure(): Unit = {
    ...
    bind[Clock].toInstance(new ClockImpl)
  }
  ...
  def oAuth2InfoDao(dbConfigProvider: DatabaseConfigProvider): DelegableAuthInfoDAO[OAuth2Info] =
    new OAuth2InfoDaoImpl(dbConfigProvider)
